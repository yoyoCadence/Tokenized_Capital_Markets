...
**P0-05 status (2026-09-26):** v2 has a separate read-only point-in-time evidence selector and UNI formula path with realized quarter, annualized run rate, modeled annual horizon and reverse requirement; see [validation](p0-05-validation.md). The v1 formula and snapshot pipeline described below still uses legacy economic dates and is not historical-as-known. Two current UNI thesis rules using legacy modeled/mixed inputs are disabled as insufficient evidence. No live v2 observations exist; rule cadence (P0-06) and replayable snapshot publication (P0-07) remain unimplemented.
