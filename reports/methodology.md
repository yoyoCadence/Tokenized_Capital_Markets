...
**P0-04 status (2026-09-26):** v2 now has a separate read-only point-in-time evidence selector with explicit economic/knowledge/valuation cutoffs and two distinct knowledge policies; see [validation](p0-04-validation.md). The v1 formula, thesis and snapshot pipeline described below still uses legacy economic dates and is not historical-as-known. The v2 selector has no live observations and is not yet connected to financial formulas (P0-05), rule cadence (P0-06), or replayable snapshot publication (P0-07).
